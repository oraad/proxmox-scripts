...
APP="Metadata Remote"
NSAPP="metadata-remote"
var_tags="${var_tags:-audio;music;metadata;media;docker}"
var_cpu="${var_cpu:-1}"
...
variables
NSAPP="metadata-remote"
var_install="metadata-remote-install"
color
catch_errors

function metadata_remote_primary_ipv4() {
...
function metadata_remote_show_url() {
  local ip
  ip="$(metadata_remote_primary_ipv4)"
  echo -e "${INFO}${YW} Access Metadata Remote at:${CL}"
  echo -e "${TAB}${GATEWAY}${BGN}http://${ip}:8338${CL}"
}
...
function update_script() {
  ...
  if [[ ! -f /opt/metadata-remote/compose.yaml ]]; then
  ...
  if [[ "$UPD" == "1" ]]; then
    ...
    cd /opt/metadata-remote
    $STD docker compose pull
    $STD docker compose up -d
    docker inspect ghcr.io/wow-signal-dev/metadata-remote:latest --format='{{index .RepoDigests 0}}' 2>/dev/null \
      | awk -F@ '{print $2}' > /opt/metadata-remote/metadata_remote_version.txt || true
    msg_ok "Updated ${APP}"

    msg_ok "Updated successfully!"
    metadata_remote_show_url
    exit
  fi
...
echo -e "${INFO}${YW} Music library bind-mounted to:${CL} /media (host: ${var_media_path:-none})"
metadata_remote_show_url